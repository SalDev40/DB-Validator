...
class Database:
    ...
    """
                ...
                self.conn.commit()
                print('DB REFINT CHECK HAD ERRORS SKIPPING TABLE\n', e)
    ...
    def checkNormal(self):
        for table in self.schema:
            try:
                print("\n**************************\
                    \n DB normal checking table:", table,
                      "\n**************************")
                ...
                for npa in self.schema[table]['nonPrimaryAttributes']:
                    print('--> checking npa: ', npa)

                    # if column has no duplicates than skip it
                    self.cursor.execute(
                        sql.SQL(
                            """
    ...
    """
                    ...
                    if self.cursor.rowcount == 0:
                        print("col: ", npa, " has no duplicates")
                        self.schema[table]['normalized'] = True
                        continue
    ...
    """
                        ...
                        if(finalResult == False):
                            # if not normal break out of loop  whole table fails
                ...
                print('DB NORMAL COMMANDS HAD ERRORS SKIPPING TABLE:\n', e)
                continue
